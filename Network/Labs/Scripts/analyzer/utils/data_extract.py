...
#Mail: POP3 src 110, IMAP src 143,SMTP des 25
...
def client_info(PCAPS):
    with open(get_path('warning/CLIENT_INFO'), 'r', encoding='UTF-8') as f:
        lines = f.readlines()
    ...
    for sess, payload in allpayloads_dict.items():
        ...
        times = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(int(pcap.time)))
...
def sen_data(PCAPS, host_ip):
    ...
    #HTTP
    with open(get_path('warning/HTTP_DATA'), 'r', encoding='UTF-8') as f:
        lines = f.readlines()
    user = lines[0].strip()
