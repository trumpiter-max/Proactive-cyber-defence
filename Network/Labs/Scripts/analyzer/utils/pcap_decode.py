...
class PcapDecode:
    def __init__(self):
        with open(get_path('protocol/ETHER'), 'r', encoding='UTF-8') as f:
            ethers = f.readlines()
        ...
        with open(get_path('protocol/IP'), 'r', encoding='UTF-8') as f:
            ips = f.readlines()
        self.IP_DICT = dict()
        for ip in ips:
            ip = ip.strip().strip('\n').strip('\r').strip('\r\n')
        ...
        with open(get_path('protocol/PORT'), 'r', encoding='UTF-8') as f:
            ports = f.readlines()
        self.PORT_DICT = dict()
        for port in ports:
        ...
        with open(get_path('protocol/TCP'), 'r', encoding='UTF-8') as f:
            tcps = f.readlines()
        self.TCP_DICT = dict()
        for tcp in tcps:
            tcp = tcp.strip().strip('\n').strip('\r').strip('\r\n')
        ...
        with open(get_path('protocol/UDP'), 'r', encoding='UTF-8') as f:
            udps = f.readlines()
        self.UDP_DICT = dict()
        for udp in udps:
            udp = udp.strip().strip('\n').strip('\r').strip('\r\n')
