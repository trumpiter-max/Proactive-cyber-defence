    ...
    for web in webdata:
        ...
        for raw_data, data in zip(raw_data_list, data_list):
            if start:
                ...
                with open(get_path(folder + file_name), 'wb') as f:
    ...
    for mail in maildata:
        ...
        for filename, filedata in file_dict.items():
            ...
            if isinstance(filedata, str):
                ...
                mode = 'wb'
                encoding = None
            with open(get_path(folder+filename), mode, encoding=encoding) as f:
                f.write(filedata)
...
def all_files(PCAPS, folder):
    file_header = dict()
    with open(get_path('protocol/FILES'), 'r', encoding='UTF-8') as f:
        lines = f.readlines()
    for line in lines:
        ...
        for data in datas:
            ...
            for header, suffix in file_header.items():
                if d.startswith(header.encode('UTF-8')):
                    ...
                    with open(get_path(folder+filename), 'wb') as f:
